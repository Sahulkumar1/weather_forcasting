...
fetch('https://api.openweathermap.org/data/2.5/forecast?q='+newName.value+'&appid=c7d0ec0826e6b918106e528e304de97b')
...
.then(data => {
    ...
    for(i = 0; i<5; i++){
        ...
        document.getElementById("day" + (i+1) + "wind").innerHTML = "Wind Speed: " + Number(data.list[i].wind.speed).toFixed(2);
        document.getElementById("day" + (i+1) + "disc").innerHTML = (data.list[i].weather[0].description).toUpperCase();
...
document.getElementById("date").innerHTML=(d.getDate()).toString()+" / "+(month[d.getMonth()]);
var a=d.getHours();
if(a>12){
  document.getElementById("time").innerHTML=(a-12).toString()+" : "+(d.getMinutes()).toString()+" PM";
}
else{
  document.getElementById("time").innerHTML=(d.getHours()).toString()+" : "+(d.getMinutes()).toString()+" AM";
}
